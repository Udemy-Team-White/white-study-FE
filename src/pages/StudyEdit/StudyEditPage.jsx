...
import { usePostStudyId } from "../../api/queries/usePostStudyId";
...
const StudyEditPage = () => {
  ...
  useEffect(() => {
    ...
    const info = studyData.studyInfo;

    reset({
      studyName: info.studyName || "",
      title: info.title || "",
      content: info.content || "",
      studyType: info.studyType || "ONLINE",
      maxMembers: info.maxMembers || "",
      closedAt: info.closedAt ? new Date(info.closedAt) : null,
      startDate: info.startDate ? new Date(info.startDate) : null,
      endDate: info.endDate ? new Date(info.endDate) : null,
    });

    setCategories(studyData.categories?.map((c) => c.name) || []);
  }, [studyData, reset]);

  const [categories, setCategories] = useState([]);
  const [selectedCategory, setSelectedCategory] = useState();

  const { mutate: editStudy, isLoading } = usePostStudyId();
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <LineBox $grid={isDesktop ? "4fr 2fr 2fr" : "1fr 1fr"}>
        ...
        <LilacButton type="submit">스터디 정보 수정</LilacButton>
      </LineBox>
    </form>
  );
};
